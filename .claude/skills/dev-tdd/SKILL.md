---
name: dev-tdd
description: TDD development workflow. Plan → write tests first → write code → run tests → fix loop → review → verify. Uses sde, test-eng, run-tests, and python-reviewer.
color: blue
---

# Dev TDD — Test-Driven Development Workflow

Full TDD cycle: plan the feature, write tests first, then write code to make them pass, review, and verify.
...
### Phase 2: Write Tests (test-eng)

Use the `Agent` tool:
- `subagent_type`: `test-eng`
- `prompt`: Pass the approved plan. Ask test-eng to write comprehensive tests for the planned feature — covering happy path, error cases, and edge cases. The tests should be written against the planned API (function signatures from the plan), not existing code.

### Phase 3: Run Tests (expect failures)

Invoke `/run-tests` using the Skill tool with the target directory. All tests should fail at this point (red phase). This confirms the tests are actually testing something.

### Phase 4: Write Code (sde)

Use the `Agent` tool:
- `subagent_type`: `sde`
- `prompt`: Pass the approved plan AND the test files that were written. Ask sde to implement the feature so that all tests pass. Tell it NOT to modify any test files.

### Phase 5: Test Loop (max 3 iterations)
...
### Phase 6: Code Review Loop (max 3 iterations)

1. Invoke `/python-reviewer` using the Skill tool.
2. If verdict is **APPROVE** → proceed to Phase 7.
...
- Write tests BEFORE code (true TDD)
